function [ systemMatrix ] = generateSystemMatrix(h1,h2,d,num,length,width,M,N,angNum )
%GENERATESYSTEMMATRIX Generate system matrix arrording to the geometrical
%                     configuration
...
rowPointer = 1;
% flag=1;
for angTemp = 0:2*pi/angNum:pi*2-2*pi/angNum
    [sourPos,endPos] = generateIndices(h1,h2,d,num,angTemp);
    
    for endNum = 1:num
        endSinglePos = endPos(endNum,:);
        [~,~,~,systemMatrixRow] = extractRadiPathSiddon(sourPos,endSinglePos,xPlane,yPlane);
